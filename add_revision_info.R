...
log <- read_csv("data_out/all_data.csv", locale = locale(encoding = "Latin1"))
...
logs_extended <- log %>%
  ...
  select(ParticipantID, id, type, output, positionFull, doclengthFull, charProduction,
  ...
  mutate(
    ...
    deletion = ifelse((
      ...
      output %in% c("BACK", "DELETE") | lead(doclengthFull) < doclengthFull) |
        #selection + backspace
        (type == "replacement" & lead(output) %in% c("BACK", "DELETE") & 
    ...
    deletion = ifelse((type == "replacement" &
                         lag(output) %in% c("BACK", "DELETE") & deletion == 1) | 
                        (((type == "replacement" & 
                             lag(type) == "replacement") | 
                            (output %in% c("BACK", "DELETE") & 
                               lag(output) %in% c("BACK", "DELETE"))) &
...
revisions <- logs_extended3 %>%
  ...
  mutate(deleted_char = ifelse(deleted_char %in% c("SPACE", "TAB", "RETURN"), 
                               "_", deleted_char)) %>%
  group_by(ParticipantID, revision_no) %>%
  summarize(
    length = n(),
    start_id = first(id),
...
text_production <- logs_extended3 %>%
  ...
  mutate(output = ifelse(output %in% c("SPACE", "TAB", "RETURN"), 
                         "_", output)) %>%
  group_by(ParticipantID, text_prod_no) %>%
  summarize(
...
revision_annotation <- full_join(revisions, text_production, 
                                 by = c("ParticipantID","revision_no")) %>%
  arrange(ParticipantID, revision_no) %>%
...
write.csv(revision_annotation, "data_out/revision_annotation.csv")
write.csv(revisions, "data_out/revisions.csv")
write.csv(text_production, "data_out/text_production.csv")
write.csv(logs_extended3, "data_out/logs_extended3.csv")
...
logs_extended6 <- logs_extended3 %>%
  ...
  left_join(revision_annotation, by = join_by(ParticipantID, revision_no)) %>%
  group_by(ParticipantID) %>%
  ...
  fill(typed_text) %>%
  fill(removed_chars) %>%
  mutate(removed_chars = ifelse(revision_no == 0, NA, removed_chars)) %>%
  # add mouse data back in
  full_join(log) %>%
  arrange(ParticipantID, id, rev_char_no)

# write extended log file for automated annotation
write.csv(logs_extended6, "data_out/all_logs_extended.csv", row.names = F)
